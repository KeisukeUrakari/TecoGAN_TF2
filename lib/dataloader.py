...
def loadHR_batch(FLAGS, tar_size):
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.compat.v1.variable_scope('load_frame'):
            ...
            tensor_set_lists = tf.convert_to_tensor(value=image_set_lists, dtype=tf.string)
        ...
        with tf.compat.v1.name_scope('sequence_data_preprocessing'):
            ...
            if(FLAGS.movingFirstFrame): 
                ...
                min_pos = tf.reduce_min( input_tensor=pos_xy, axis=0 )
                range_pos = tf.reduce_max( input_tensor=pos_xy, axis=0 ) - min_pos # [ shrink x, shrink y ]
            ...
            for fi in range( FLAGS.RNN_N ):
                ...
                if FLAGS.random_crop and FLAGS.mode == 'train':
                    ...
                    if FLAGS.movingFirstFrame:
                        ...
                        HR_sequence_crop = tf.compat.v1.where(moving_decision < 0.7, HR_sequence_crop, HR_sequence_crop_1)
                        name_sequence = tf.compat.v1.where(moving_decision < 0.7, name_sequence, name_sequence1) 
...
def loadHR(FLAGS, tar_size):
    ...
    with tf.device('/cpu:0'):
        ...
        image_list_HR_r = [tf.convert_to_tensor(value=_ , dtype=tf.string) for _ in image_list_HR_r ]
            ...
            for fi in range( FLAGS.RNN_N ):
                ...
                if(FLAGS.movingFirstFrame):
                    if(fi == 0):
                        ...
                        target_size = tf.shape(input=HR_data_0)
                    ...
                    HR_data = tf.cond(pred=moving_decision < 0.7, true_fn=lambda: tf.identity(HR_data), false_fn=lambda: tf.identity(HR_data_1))
                    output_names[fi] = tf.cond(pred=moving_decision < 0.7, true_fn=lambda: tf.identity(output_names[fi]), false_fn=lambda: tf.identity(output_names[0]))
                data_list_HR_r.append( HR_data )
            
            target_images = data_list_HR_r
        
        # Other data augmentation part
        with tf.compat.v1.name_scope('data_preprocessing'):
            
            with tf.compat.v1.name_scope('random_crop'):
                # Check whether perform crop
                if (FLAGS.random_crop is True) and FLAGS.mode == 'train':
                    print('[Config] Use random crop')
                    target_size = tf.shape(input=target_images[0])
...
def frvsr_gpu_data_loader(FLAGS, useValData_ph): # useValData_ph, tf bool placeholder, whether to use validationdata
    ...
    with tf.compat.v1.name_scope('load_frame_cpu'):
        with tf.compat.v1.name_scope('train_data'):
            print("Preparing train_data")
            batch_list, num_image_list_HR_t_cur = loadHRfunc(FLAGS, tar_size)
        with tf.compat.v1.name_scope('validation_data'):
    ...
    target_images = []
    input_images = []
    with tf.compat.v1.name_scope('load_frame_gpu'):
        with tf.device('/gpu:0'):
            for frame_t in range(FLAGS.RNN_N):
                ...
                curHR = tf.cond( pred=useValData_ph, true_fn=getValdHR, false_fn=getTrainHR )
    ...
    curHRpath = tf.cond( pred=useValData_ph, true_fn=getValdHRpath, false_fn=getTrainHRpath )
