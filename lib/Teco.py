...
def VGG19_slim(input, reuse, deep_list=None, norm_flag=True):
    ...
    with tf.device('/gpu:0'):
        ...
        # feature maps:
        results = {}
        with tf.compat.v1.name_scope('vgg_norm'):
            for key in output:
                if (deep_list is None) or (key in deep_list):
                    orig_deep_feature = output[key]
                    if norm_flag:
                        orig_len = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(orig_deep_feature), axis=[3], keepdims=True)+1e-12)
...
def TecoGAN(r_inputs, r_targets, FLAGS, GAN_Flag=True):
    ...
    with tf.compat.v1.variable_scope('generator'):
        ...
        for frame_i in range( inputimages - 1 ):
            ...
            gen_pre_output_reshape = tf.transpose( a=gen_pre_output_reshape, perm = [0,1,3,2,4,5] )
    ...
    if FLAGS.vgg_scaling > 0.0:
        with tf.compat.v1.name_scope('vgg_19') as scope:
    ...
    if(GAN_Flag):# build the discriminator
        # prepare inputs
        with tf.device('/gpu:0'), tf.compat.v1.name_scope('input_Tdiscriminator'):
        ...
        with tf.compat.v1.name_scope('real_Tdiscriminator'):
            ...
            with tf.device('/gpu:0'), tf.compat.v1.variable_scope('tdiscriminator', reuse=False):
                ...
                if(FLAGS.Dt_mergeDs): # a spatio-temporal D
                    if(FLAGS.crop_dt < 1.0): real_warp = tf.pad(tensor=real_warp, paddings=paddings, mode="CONSTANT" )
                    with tf.compat.v1.variable_scope('sdiscriminator', reuse=False): # actually no more variable under this scope... 
                        before_warp = tf.reshape(t_targets, (t_batch, 3, FLAGS.crop_size*4, FLAGS.crop_size*4, 3))#[tb,3=T,h,w,3=RGB]
                        before_warp = tf.transpose(a=before_warp, perm=[0, 2, 3, 4, 1]) # [b,h,w,3RGB,3T]
                        ...
                        t_input = tf.transpose(a=t_input, perm=[0, 2, 3, 4, 1]) # [tb,h//4,w//4,3RGB,3T]
                        t_input = tf.reshape(t_input, (t_batch, FLAGS.crop_size, FLAGS.crop_size,-1) ) # [tb,h//4,w//4,9=RRRGGGBBB]
                        input_hi = tf.image.resize( t_input, (FLAGS.crop_size*4, FLAGS.crop_size*4) )# [tb,h,w,9=RRRGGGBBB]
        ...
        with tf.compat.v1.name_scope('fake_Tdiscriminator'):
            fake_warp0 = tf.contrib.image.dense_image_warp(t_gen_output, T_vel)
            with tf.device('/gpu:0'), tf.compat.v1.variable_scope('tdiscriminator', reuse=True): # reuse weights
                fake_warp = tf.reshape(fake_warp0, (t_batch, 3, FLAGS.crop_size*4, FLAGS.crop_size*4, 3))
                fake_warp = tf.transpose(a=fake_warp, perm=[0, 2, 3, 4, 1])
                ...
                if(FLAGS.Dt_mergeDs): # a spatio-temporal D
                    if(FLAGS.crop_dt < 1.0): fake_warp = tf.pad(tensor=fake_warp, paddings=paddings, mode="CONSTANT" )
                    with tf.compat.v1.variable_scope('sdiscriminator', reuse=True): # actually no more variable under this scope... 
                        before_warp = tf.reshape(t_gen_output, (t_batch, 3, FLAGS.crop_size*4, FLAGS.crop_size*4, 3))
                        before_warp = tf.transpose(a=before_warp, perm=[0, 2, 3, 4, 1])
        ...
        if(FLAGS.D_LAYERLOSS): 
            ...
            with tf.device('/gpu:0'), tf.compat.v1.variable_scope('layer_loss'):
                ...
                for layer_i in range(layer_n):
                    ...
                    layer_loss = tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tf.abs(layer_diff), axis=[3]))
                    # an l1 loss
    ...
    with tf.compat.v1.variable_scope('generator_loss'):
        ...
        with tf.device('/gpu:0'), tf.compat.v1.variable_scope('content_loss'):
            ...
            content_loss = tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tf.square(diff1_mse), axis=[3]))
        ...
        with tf.compat.v1.variable_scope('warp_loss'):
            ...
            warp_loss = tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tf.square(diff2_mse), axis=[3]))
        ...
        if FLAGS.vgg_scaling > 0.0:
            with tf.device('/gpu:0'), tf.compat.v1.variable_scope('vgg_layer_loss'):
                ...
                for layer_i in range(vgg_layer_n):
                    curvgg_diff = tf.reduce_sum(input_tensor=gen_vgg[vgg_layer_labels[layer_i]]*target_vgg[vgg_layer_labels[layer_i]], axis=[3])
                    # cosine similarity, -1~1, 1 best
                    curvgg_diff = 1.0 - tf.reduce_mean(input_tensor=curvgg_diff) # 0 ~ 2, 0 best
        ...
        if FLAGS.pingpang:
            with tf.device('/gpu:0'), tf.compat.v1.variable_scope('bidirection_loss'):
                ...
                # an l1 loss
                pploss = tf.reduce_mean(input_tensor=tf.abs(gen_out_first - gen_out_last_rev))
        ...
        if(GAN_Flag): # spatio-temporal adversarial loss
            with tf.compat.v1.variable_scope('t_adversarial_loss'):
                t_adversarial_loss = tf.reduce_mean(input_tensor=-tf.math.log(tdiscrim_fake_output + FLAGS.EPS))
    ...
    if(GAN_Flag): # Build the discriminator loss
        with tf.device('/gpu:0'), tf.compat.v1.variable_scope('t_discriminator_loss'):
            ...
            t_discrim_loss = tf.reduce_mean(input_tensor=-(t_discrim_fake_loss + t_discrim_real_loss))\
            # a criterion of updating Dst
            t_balance = tf.reduce_mean(input_tensor=t_discrim_real_loss) + t_adversarial_loss
            ...
            update_list += [tf.reduce_mean(input_tensor=tdiscrim_real_output), tf.reduce_mean(input_tensor=tdiscrim_fake_output)]
        ...
        with tf.device('/gpu:0'), tf.compat.v1.variable_scope('tdicriminator_train'):
            tdiscrim_tvars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='tdiscriminator')
            ...
            tdiscrim_optimizer = tf.compat.v1.train.AdamOptimizer(tdis_learning_rate, beta1=FLAGS.beta, epsilon=FLAGS.adameps)
    ...
    with tf.compat.v1.variable_scope('generator_train'):
        gen_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate, beta1=FLAGS.beta, epsilon=FLAGS.adameps)
        fnet_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate, beta1=FLAGS.beta, epsilon=FLAGS.adameps)
        gen_tvars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='generator')
        fnet_tvars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='fnet')
        ...
        if(not GAN_Flag):
            ...
            with tf.control_dependencies(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)):
                ...
                def train_gen_withD():
                    ...
                    with tf.control_dependencies([ tf.compat.v1.assign_add(counter1, 1), update_tb, tdiscrim_train ]):
                ...
                # train D adaptively
                gen_train, fnet_train, gen_grads_and_vars, fnet_grads_and_vars = \
                     tf.cond( pred=tf.less(tb, FLAGS.Dbalance), true_fn=train_gen_withD, false_fn=train_gen_withoutD )
                update_list_avg += [counter1, counter2]
                update_list_name += ["withD_counter", "w_o_D_counter"]
            
    with tf.compat.v1.name_scope('image_summaries'):
